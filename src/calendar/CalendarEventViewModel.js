			...
			const makeGuestList = (model) => {
				return model.bccRecipients().map(recipientInfo => {
					const guest = {
						...
						password: model.getPassword(recipientInfo.mailAddress),
					}
					recipientInfo.resolveContactPromise && recipientInfo.resolveContactPromise.then(contact => {
						guest.password = model.getPassword(guest.address.address)
					})
					return guest
				})
			}
	...
	getPasswordStrength(guest: Guest): number {
		...
		const getStrength = (model) => {
			const recipient = model.allRecipients()
			                       .find(r => address === r.mailAddress && password === model.getPassword(address))
...
export function createCalendarEventViewModel(date: Date, calendars: Map<Id, CalendarInfo>, mailboxDetail: MailboxDetail,
	...
	return new CalendarEventViewModel(
		...
		mailboxDetail,
		(mailboxDetail) => new SendMailModel(worker, logins, locator.mailModel, locator.contactModel, locator.eventController, locator.entityClient, mailboxDetail),
		date,
		getTimeZone(),
		calendars,
		existingEvent,
